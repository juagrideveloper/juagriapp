...
fun Timestamp?.toDisplayTimeStamp() = GMTDate(this.value().toLong()).toDisplayTimeStamp()

fun Timestamp?.toMMYYYY() = GMTDate(this.value().toLong()).toMMYYYY()
...
fun GMTDate.toDisplayTimeStamp(): String = buildString {
    append(dayOfMonth.padZero(2))
    append("-")
    append(month.ordinal.plus(1).padZero(2))
    append("-")
    append(year.padZero(4))
    append(" ")
    append(hours.padZero(2))
    append(":")
    append(minutes.padZero(2))
    append(if (hours >= 11) "AM" else "PM")
}

fun GMTDate.toMMYYYY(): String = buildString {
    append(month.ordinal.plus(1).padZero(2))
    append("-")
    append(year.padZero(4))
}
